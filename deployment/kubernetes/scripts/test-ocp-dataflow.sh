...
verify_recommendations() {
    ...
    # Check for actual recommendations in database
    echo_info "Checking for recommendations in database..."
    ...
    if [ -n "$db_pod" ]; then
        ...
        if [ "$rec_count" -gt 0 ]; then
            echo_success "✓ Found $rec_count recommendation(s) generated by Kruize"

            # Show actual recommendation details
            echo_info "Latest recommendation summary:"
            ...
            echo_success "✓ Recommendations successfully generated and saved"
        else
            ...
            echo_info "  - Insufficient data for analysis"
        fi
    else
...
case "${1:-}" in
    ...
    "help"|"-h"|"--help")
        ...
        echo "  - kruize:      localhost:8080  -> recommendations"
        exit 0
        ;;
    "")
        main
